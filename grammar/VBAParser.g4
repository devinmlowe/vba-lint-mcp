...
// Source: https://github.com/rubberduck-vba/Rubberduck (commit: fae50adab188126a5e7d2a1cefc3328cc18af482)
// License: GPL-3.0
...
options {
    tokenVocab = VBALexer;
    // superClass and contextSuperClass removed — these are Rubberduck-specific
    // base classes (VBABaseParser, VBABaseParserRuleContext) not needed for standalone use.
 }
...
// Modified: use individualNonEOFEndOfStatement instead of endOfStatement to avoid
// ANTLR4 error 186 (closure matching EOF). endOfStatement can match EOF which
// creates an infinite loop in the closure. This is safe because unterminated blocks
// are always followed by a keyword (NEXT, WEND, LOOP, etc.) not EOF.
